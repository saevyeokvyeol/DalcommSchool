...
<script type="text/javascript">
	$(function() {
		...
		$(".updateForm").click(function() {
			...
			$.ajax({
				url : "${pageContext.request.contextPath}/eventCouponUpdateForm",
				type : "post",
				data: {
				...
				success : function(result) {
					//alert(result)
					$("#coupon-admin-update .couponId").val(`\${result.couponId}`);
					$("#coupon-admin-update .couponName").val(`\${result.couponName}`);
					$("#coupon-admin-update .couponDc").val(`\${result.couponDc}`);
					$("#coupon-admin-update .couponEndDate").val(`\${result.couponEndDate}`);
				},
							...
							<span class="pagination-inner"> 
							  <c:forEach var='i' begin='${startPage}' end='${(startPage-1)+blockCount}'> 
							    ...
							    <c:if test="${not doneLoop}" >
							         <a class="${i==nowPage?'pagination-active':page}" href="${pageContext.request.contextPath}/admin/coupon/selectAllCoupon?nowPage=${i}">${i}</a> 
							    </c:if>
