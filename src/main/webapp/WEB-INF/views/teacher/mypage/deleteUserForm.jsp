...
$(function() {
	...
	$(document).ready(function(){
	  ...
	  $(document).on("click", "button[name=Withdrawal]", function() {
			  $.ajax({
					url:"${pageContext.request.contextPath}/checkTeacherPwd",
...
<body>
	...
	<!--       -----------------------------탈퇴 폼------------------------------------         -->
		...
		<form id="delete" name="deleteInfo" action="${pageContext.request.contextPath}/main/mypage/deleteTeacher" method="post" class="row g-3">
			<input type="hidden" name="${_csrf.parameterName}" value="${_csrf.token}"> <!-- csrf token 전송 -->
			<input type="hidden" id="userId" name="userId" value="${teacher.teacherId}" />
